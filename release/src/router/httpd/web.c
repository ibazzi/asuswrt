...
{
	int ret = 0, customList_status = 0, client_nickname_status = 0;
...
static int validate_apply(webs_t wp, json_object *root) {
	...
	if(acc_modified){
		...
		if(strlen(modified_pass) <= 0){
			...
			int declen3 = pw_dec_len(http_passwd_t);
			char dec_passwd[declen3];
...
static int ej_update_variables(int eid, webs_t wp, int argc, char_t **argv) {
	    ...
	    !strcmp(action_mode, "apply_new"))
	{
		...
		if(do_apply || has_modify) {
			...
			if (!strcmp(action_script, "restart_net_and_phy") || !strcmp(action_script, "restart_all")) {
				websWrite(wp, "<script>restart_needed_time(%d);</script>\n",
...
#endif			
#if defined(RTCONFIG_QCA)
			if (!strcmp(action_script, "restart_allnet")) {
				websWrite(wp, "<script>restart_needed_time(%d);</script>\n",
...
static void
do_alexa_pause_internet_cgi(char *url, FILE *stream)
{
	struct json_object *root=NULL;
	char *pause_internet = NULL;

	root = json_object_new_object();

	do_json_decode(&root);

	pause_internet = safe_get_cgi_json("pause_internet", root);

	if(!strcmp(pause_internet, "1"))
		alexa_pause_internet(1);
	else if(!strcmp(pause_internet, "0"))
		alexa_pause_internet(0);
...
struct mime_handler mime_handlers[] = {
	...
	{ "alexa_pause_internet.cgi", "text/html", no_cache_IE7, do_html_post_and_get, do_alexa_pause_internet_cgi, do_auth },
#ifdef RTCONFIG_NOTIFICATION_CENTER
...
static int ej_netdev(int eid, webs_t wp, int argc, char_t **argv)
{
 FILE * fp;
  ...
  if ((fp = fopen(PPACMD_TRAFFIC_PATH, "r")) != NULL) {
		comma = ' ';
			while (fgets(buf, sizeof(buf), fp)) {
				printf("%s\n", buf);
				if ((p = strchr(buf, '[')) == NULL) continue;

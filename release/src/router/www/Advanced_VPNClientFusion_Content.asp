...
function del_Row(rowdata) {
	...
	if(exist_exception_list_flag) {
		if(!confirm("All exception policies of deleted VPN profile will be removed. Are you sure you want to delete?"))/*untranslated*/
			return false;
	}
	else {
...
function edit_exception_list_confirm(_parArray) {
	...
	if(activate_status == "1") {
		alert("Please deactivate before editing.");/*untranslated*/
		return false;
	}
...
function del_exception_list_confirm(_parArray) {
	...
	if(activate_status == "1") {
		...
		if(manually_dhcp_list_array_ori[ipAddr] != undefined) {
			if(!confirm("Remove the client's policy in the exception list of VPN Fusion will also delete the client's IP binding of DHCP Server. Are you sure you want to delete?"))/*untranslated*/
				return false;
		}
	}
